...
public class IntOperation { // class s
	...
	public static void main(String[] args) { // main s
		// 정수에서 int 이하 피연산자 간의 연산은 int 타입으로 변환되어 연산한다.
		byte b = 10;
		short s = 20;
		...
		// byte type 오버플로
		// byte b1 = 128l // byte type오르 강제 형변환해야(-128)
		// System.out.println("b1 : " + b1);
		
		// 코드 타이핑과 동시에 미리 연산을 하여 b2에 저장하려고 한다.
		// 변수가 아니라 단순 리터럴 간의 연산이므로 형병환ㅇ 없다.
		// 단지 왼쪽 변수의 범위 내인지 만 체크
//		byte b2 = 126 + 1; // byte 타입 범위 : -128 ~ +127이므로 가능
		// b2 = 126 + 2; // [오류] +128이 되어 오버플로우
//		System.out.println("add : " + b2);
		
		// 정수 변수 간의 기본적으로 int 타입으로 형변환 된 후에 연산된고 그 결과도 int 타입
...
//		i = i longVal; // type cast
		...
		// 실수가 연산(큰 타입으로 변환된 후에 연산, 결과도 큰타입)
		double dVal = 300.5;
		...
		int num1 = 9;
		int num2 = 2;
		double num3 = num1 / num2; // num1, num2 모두 int 타입이기 떄문에 연시하면 소수점 버림
		System.out.println("num3 : " + num3); // 정수간 나눗셈은 정수만 출력한다. 소수점 버림
		
		// 한쪽을 형변환하면 다른 쪽도 큰쪽으로 맞춰서 형변환된
		// 후에 연산이 되고 결과도 double 타입
		num3 = (double)num1 / num2;	// 캐스팅연산자 ( ) 더블타입으로
		System.out.println(num3);	// 4.5
		
		// 하지만 이경우는 안됨
		// (num1 / num2 ) 괄호 안에서 이미 결과가 소숫점이
		// 버려진 상태기 때문에 (double)0 해봐야 의미없음.
		double num6 = (double)(num1 / num2); // 정수연산이먼저되서 소수점이 버려짐
